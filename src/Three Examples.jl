...
using LinearAlgebra, Polynomials, Plots, PlutoUI

# ╔═╡ 2bd4e64a-85a4-4726-aad7-01938b4d7edc
md"""
# Three Examples

## Graeffe's method and `BigFloat`

Use the package `Polynomials.jl` to manipulate polynomials.
Write the function which implements 
...
Use Julia's `BigFloat` numbers to overcome the main disadvantage of the method. What is the number of significant decimal digits, and the largest and the smallest number?

Test the function on the [Wilkinson's polynomial](https://en.wikipedia.org/wiki/Wilkinson%27s_polynomial) $\omega_{20}(x)$, and the [Chebyshev polynomial](https://en.wikipedia.org/wiki/Chebyshev_polynomials) $T_{50}(x)$ 
(the latter needs to be transformed in order to apply the method).

Compare your solutions with the exact solutions.
"""

# ╔═╡ c0237d73-24e7-486e-b724-23092af66800
md"""
### Precision

The function `eps(T)` returns the smallest number of the type `T` which is larger than 1. It can be called for each of the `AbstractFloat` types. 

Functions `floatmin(T)` and `floatmax(T)` return the largest and the smallest positive numbers representable in the given type.
"""
...
# ╔═╡ 47ba6238-9b3e-4e2f-b136-03663ab6f935
...
### Wilkinson's polynomial
 ...
 $\omega_{20}$ is constructed as follows:
"""

# ╔═╡ 28003bac-9ae8-4a6d-9073-830e0c47f8fa
ω=fromroots(collect(one(BigFloat):20))

# ╔═╡ 43a81907-553e-4d3c-8119-15c4f8f59bb0
roots(ω)
...
Write the function which computes simple LU factorization (without pivoting) where the matrix is overwritten by the factors. Make sure that the function also works with block-matrices. This is an example of Julia's _multiple dispatch_ feature (_polymorphism_)
...
The key is that `/` works for block matrices, too. $A$ is overwritten and must therefore be copied at the begining of the function so that the original matrix is not overwritten.
"""
...
# ╔═╡ 4b30f7d3-9f90-4c02-b157-3506ba705674
@time mylu(Ar)
...
Are the eigenvalues random? Can you describe their behavior? Can random matrices be used to test numerical algorithms?
"""
...
begin
	...
	md"""
	k = $(@bind kₘ Slider(10:30,show_value=true)) $nbsp $nbsp
...
> Be carefull when doing tests with random data!
"""
